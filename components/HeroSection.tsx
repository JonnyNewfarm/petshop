...
const grainDataUrl = `url("data:image/svg+xml,${encodeURIComponent(
  grainSvg,
)}")`;
...
export default function HeroSection() {
  ...
  const cardImageRefs = useRef<(HTMLAnchorElement | null)[]>([]);
  ...
  useLayoutEffect(() => {
    const ctx = gsap.context(() => {
      ...
      const placeInactive = (index: number) => {
        ...
        gsap.set(cardTextRefs.current[index], {
          yPercent: 100,
          y: 0,
          opacity: 0,
          zIndex: 1,
        ...
        gsap.set(cardImageRefs.current[index], {
          yPercent: -100,
          y: 0,
          opacity: 0,
          zIndex: 1,
      ...
      if (shouldPlayPreloader) {
        ...
        gsap.set([cardTextRefs.current[0], cardImageRefs.current[0]], {
          y: 40,
          opacity: 0,
          autoAlpha: 0,
        });
        ...
        introTl
          ...
          .to(
            [cardTextRefs.current[0], cardImageRefs.current[0]],
            {
              y: 0,
              opacity: 1,
              autoAlpha: 1,
      ...
      const goToSlide = (direction: 1 | -1) => {
        ...
        const currentCardText = cardTextRefs.current[current];
        const currentCardImage = cardImageRefs.current[current];
        const currentTitle = titleRefs.current[current];

        const nextLeft = leftRefs.current[next];
        const nextRight = rightRefs.current[next];
        const nextCardText = cardTextRefs.current[next];
        const nextCardImage = cardImageRefs.current[next];
        ...
        gsap.set(currentCardText, {
          yPercent: 0,
          zIndex: 2,
          autoAlpha: 1,
        });

        gsap.set(currentCardImage, {
          yPercent: 0,
          zIndex: 2,
          autoAlpha: 1,
        });
        ...
        gsap.set(nextCardText, {
          yPercent: direction === 1 ? 100 : -100,
          zIndex: 3,
          autoAlpha: 1,
        });

        gsap.set(nextCardImage, {
          yPercent: direction === 1 ? -100 : 100,
          zIndex: 3,
          autoAlpha: 1,
        });
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-[calc(2.8rem+env(safe-area-inset-bottom))] right-4 z-50 grid h-[115px] w-[min(430px,calc(100%-2rem))] grid-cols-[minmax(0,1fr)_115px] gap-2 overflow-hidden md:bottom-8 md:right-8 md:h-[160px] md:w-[540px] md:grid-cols-[minmax(0,1fr)_160px] md:gap-3 [@media_(orientation:landscape)_and_(max-height:500px)]:bottom-4 [@media_(orientation:landscape)_and_(max-height:500px)]:right-4 [@media_(orientation:landscape)_and_(max-height:500px)]:h-[105px] [@media_(orientation:landscape)_and_(max-height:500px)]:w-[390px] [@media_(orientation:landscape)_and_(max-height:500px)]:grid-cols-[minmax(0,1fr)_105px] [@media_(orientation:landscape)_and_(max-height:500px)]:gap-2">
        <div className="relative h-full overflow-hidden">
          {slides.map((slide, index) => (
            <InfoTextCard
              key={`card-text-${slide.label}`}
              ref={(element) => {
...
const InfoTextCard = forwardRef<HTMLDivElement, { slide: Slide }>(
  function InfoTextCard({ slide }, ref) {
    return (
      <div
        ref={ref}
        style={{
        ...
        >
          <div className="flex items-start justify-between gap-3 md:gap-6 [@media_(orientation:landscape)_and_(max-height:500px)]:gap-3">
            <div>
              ...
              <h2 className="text-2xl font-semibold uppercase leading-[0.85] tracking-[-0.045em] md:text-3xl [@media_(orientation:landscape)_and_(max-height:500px)]:text-xl">
                {slide.title}
              </h2>
            </div>
            ...
            <svg
              ...
              aria-hidden="true"
              className=" h-4 w-4 shrink-0 opacity-65  md:h-5 md:w-5 [@media_(orientation:landscape)_and_(max-height:500px)]:h-4 [@media_(orientation:landscape)_and_(max-height:500px)]:w-4"
            >
              <path
          ...
          <div>
            <p className="mb-2 max-w-[13rem] text-[12px] font-medium leading-snug opacity-70 md:mb-3 md:max-w-[18rem] md:text-xs [@media_(orientation:landscape)_and_(max-height:500px)]:mb-1 [@media_(orientation:landscape)_and_(max-height:500px)]:max-w-[12rem] [@media_(orientation:landscape)_and_(max-height:500px)]:text-[9px]">
              {slide.text}
            </p>
...
const InfoImageCard = forwardRef<HTMLAnchorElement, { slide: Slide }>(
  function InfoImageCard({ slide }, ref) {
    return (
      <Link
        ref={ref}
        href={
        ...
        className="invisible absolute inset-0 block overflow-hidden border border-black/10 opacity-0 will-change-transform"
      >
        <Image
          src={slide.cardImage}
          alt={`${slide.label} collection`}
          fill
          draggable={false}
          sizes="(max-width: 767px) 115px, 160px"
